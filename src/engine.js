...
const boolFlags=['door','flowers','testimony','trusted'];
...
export function getChoices(s){
 ...
 if(n.hub)return [...[['door','礼拝堂の扉を確かめる','door'],['flowers','弔花と写真を見比べる','flowers'],['testimony','三人の行動を聞く','testimony']].filter(([f])=>!s.flags[f]).map(([,label,to])=>({label,to})),{label:'集めた事実を整理する',to:'theory'}];
 return (n.choices||[]).map(c=>({...c,disabled:!!c.need?.some(e=>!s.evidence.includes(e))}));
}
export function nextNode(s){return story[s.node].next;}
