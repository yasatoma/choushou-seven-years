import {renderRuby, rubyUnits,chapterRubyTexts,chapterRubyHistory} from './ruby.js';
...
import {characters,glossary} from './story.js';
import {Soundscape} from './audio.js';
import qrcode from './vendor/qrcode.js';
import './vendor/jsQR.js';
const $=id=>document.getElementById(id),storeKey='choushou.v1';
...
updateSettings();
function showTitle(){stopTimers();stopModes();if(state)autosave();mode='title';$('title-screen').hidden=false;$('game-screen').hidden=true;$('ending-screen').hidden=true;$('background').style.backgroundImage="url('assets/manor.png')";$('background').style.filter='';$('rain').hidden=true;$('continue-game').disabled=!db.auto; sound.setMood('unease');lastChapter='';lastPortrait='';}
...
const locationNames={manor:'汐祈館 / 島の入口',hall:'汐祈館 / 談話室',chapel:'汐祈館 / 礼拝堂',archive:'汐祈館 / 旧書庫',shore:'島の桟橋 / 夜明け',town:'本土 / 駅前通り',cafe:'本土 / 海の見えない喫茶店',study:'本土 / 直人の仕事部屋'};
function render(){
 stopTimers();choiceMode=false;$('choices').hidden=true;$('choices').replaceChildren();$('game-screen').classList.remove('has-choices');
 const n=story[state.node],l=n.lines[state.line],bg=l.bg||n.bg;
 ...
 $('chapter').textContent=n.chapter;$('location').textContent=locationNames[bg]||'';$('background').style.backgroundImage=`url('assets/${bg}.png')`;
 $('background').style.filter=n.mood==='dread'?'saturate(.7) brightness(.8)':'';$('rain').hidden=!['manor','town'].includes(bg);sound.setMood(n.mood);
 const c=characters[l.who];$('speaker').textContent=c?.name||l.who||'瀬尾 直人';$('speaker-role').textContent=c?.role||(l.who?'':'モノローグ');$('speaker').style.color=c?.color||'#d5deda';
 const face=c?.image?c:characters[n.lines.slice(0,state.line+1).findLast(x=>characters[x.who]?.image)?.who];
 const portrait=face?.image;
 if(portrait){if(lastPortrait!==portrait){$('portrait').src=portrait;lastPortrait=portrait;}$('portrait-wrap').style.opacity='1';}
 ...
 const speed=skip?0:db.settings.speed;const displayText=chapterRubyTexts(story,glossary,state.trail).get(l.id)||l.text;
 if(speed===0||db.settings.reduced){renderRuby($('text'),displayText);completeLine();}
 else{
  const units=rubyUnits(displayText);let count=0;renderRuby($('text'),displayText,0);
  typing=setInterval(()=>{count++;renderRuby($('text'),displayText,count);if(count>=units.length){clearInterval(typing);typing=null;completeLine();}},speed);
 }
...
function reveal(){if(!typing)return;clearInterval(typing);typing=null;const l=story[state.node].lines[state.line];renderRuby($('text'),chapterRubyTexts(story,glossary,state.trail).get(l.id)||l.text);completeLine();}
function advance(){
...
function openPanel(type){
 const titles={menu:'夜のしおり',save:'記録を残す',load:'記録をひらく',settings:'読み方を整える',backlog:'これまでの言葉',evidence:'記録帳',endings:'結末の栞',transfer:'物語を、別の端末へ',about:'この作品について',chapters:'章から読み返す'};
 ...
 if(type==='menu'){
  ...
  body.append(el('p','設定はこのブラウザに保存されます。音は開始操作のあとに流れます。','panel-note'),el('p','雨の夜、七年ぶりに鐘が鳴る。','preview-text'));
  for(const [key,label,min,max,step,unit]of [['speed','文字の表示間隔',0,70,1,' ms（0で一括）'],['fontSize','文字サイズ',17,30,1,' px'],['music','音楽',0,100,1,' %'],['ambient','雨と海の環境音',0,100,1,' %'],['effects','効果音',0,100,1,' %'],['autoDelay','オートの待ち時間',600,7000,100,' ms ＋ 文章の長さ']]){
  ...
  const rubyLog=chapterRubyHistory(story,glossary,state?.trail||[]);for(const {id,text} of rubyLog){const l=linesById[id],item=el('article',undefined,'log-item');item.append(el('strong',characters[l.who]?.name||l.who||'瀬尾 直人 · モノローグ'),rubyParagraph(text));body.append(item);}requestAnimationFrame(()=>$('panel').scrollTop=$('panel').scrollHeight);
 }else if(type==='evidence'){
  body.append(el('p','今の進行で確認した事実だけを記録しています。解釈と事実を、分けて読むために。','panel-note'));
  const map=el('div',undefined,'map');['洋館｜五人の追悼会','礼拝堂｜内側の閂','旧書庫｜七年前の火災','桟橋｜嵐が去るまで'].forEach(t=>map.append(el('div',t)));body.append(map);
  ...
  const box=el('div',undefined,'about');box.innerHTML='<h3>弔鐘は七年前を告げる</h3><p>孤島の洋館を舞台にした、オリジナル・ミステリービジュアルノベルです。調査と行動によって物語が分かれます。好感度などの数値はありません。</p><p>殺人、火災、喪失と家族による束縛を扱います。流血の画像や突然の大音量演出はありません。音に関する手掛かりは、すべて文章でも示されます。</p><h3>操作</h3><dl><dt>クリック / タップ / Enter / Space</dt><dd>文章を表示、もう一度で次へ</dd><dt>Esc</dt><dd>メニュー / パネルを閉じる</dd><dt>L / S / A</dt><dd>履歴 / セーブ / オート</dd><dt>既読スキップ</dt><dd>読んだ文章だけを送り、選択肢や未読で止まります</dd></dl><h3>保存と引き継ぎ</h3><p>手動6枠と自動1枠。ブラウザのデータ削除やプライベートモードにご注意ください。端末を替える前に引き継ぎコードを保存できます。コードには現在位置・調査・履歴・既読・読了記録を含みます。手動保存枠・設定・章の栞は端末ごとです。</p><h3>制作</h3><p>企画・脚本・プログラム・美術ディレクション：こでちゃん（Codex）<br>背景・人物：本作用に画像生成したオリジナル素材<br>音楽・環境音・効果音：本作用のWeb Audio合成音<br>エンジン：HTML / CSS / JavaScript</p><p>本作はフィクションです。登場する人物・施設・事件は架空のものです。文章・音楽・画像は本作用の新作です。保存などのシステムは前作をもとに改良しています。音声の台詞読み上げはありません。</p><p>v1.0 · SEVEN YEARS<br>広告・アクセス解析・外部素材の読込はありません。</p>';
  body.append(box);
 }
}
...
function rubyParagraph(text){const p=document.createElement("p");renderRuby(p,text);return p;}
